...
- [ ] **分拆 `cowen-common` 模块**: 该模块目前由于承载了配置、安全、网络及模型，已成为“上帝模块”。应将其底层工具剥离至 `cowen-infra` 或 `cowen-utils`，确保 `cowen-common` 仅包含核心模型和 SPI 契约，以减少不必要的编译依赖。
