...
WHEN 分发消息时
THEN SDK SHALL 使用配置的 `encryptKey` 进行 AES-128-ECB 解密
AND 将解密后的 JSON 转换为对应的消息对象。

#### Scenario: 嵌套业务结构解析
GIVEN 收到 `APP_TICKET` 或 `TEMP_AUTH_CODE` 消息
WHEN 解密成功后
THEN SDK SHALL 能够从 `bizContent` 字段中正确提取核心业务数据（如 `appTicket` 或 `tempAuthCode`）。
