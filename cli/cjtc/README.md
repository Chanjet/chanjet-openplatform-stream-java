# cjtc - Open Streaming Connector CLI

cjtc 是 Open Streaming Connector 的命令行客户端，支持完整的系统治理、API 调用模拟以及基于大语言模型的本地语义搜索功能。
...
**注意**：macOS 专用的静态库 `libtokenizers.a` 位于 `cli/cjtc/lib/` 目录下。

```bash
cd cli/cjtc
CGO_LDFLAGS="-L$(pwd)/lib -L$(pwd)/pkg/search/assets/lib/darwin-arm64" go build -o build/cjtc-darwin-arm64 ./cmd/cjtc
```
...
go build -o build/cjtc-windows-amd64.exe ./cmd/cjtc
```
...
CGO_LDFLAGS="-L$(pwd)/pkg/search/assets/lib/linux-amd64" go build -o build/cjtc-linux-amd64 ./cmd/cjtc
```
