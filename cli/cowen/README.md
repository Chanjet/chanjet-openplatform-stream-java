# 畅捷通 Stream Connector CLI (Rust 版) - owenc

[![Rust](https://img.shields.io/badge/rust-1.75%2B-orange.svg)](https://www.rust-lang.org)
[![License](https://img.shields.io/badge/license-Apache%202.0-blue.svg)](LICENSE)

`owenc` 是畅捷通开放平台（Chanjet Open Platform）官方推出的高性能、安全且智能的命令行治理工具。它旨在取代原有的 Go 版本，通过 Rust 的内存安全特性、零成本抽象以及原生 AI 能力，为开发者提供全生命周期的 API 治理体验。
...
make build
# 安装二进制到系统路径 (可选)
cp ../../bin/owenc /usr/local/bin/
```

### 2. 初始化环境 (必须包含 AppKey, Secret 与 Certificate)

```bash
owenc init --app-key YOUR_KEY --app-secret YOUR_SECRET -c YOUR_CERT
```
...
```bash
# 语义搜索接口
owenc api list -s "创建订单"

# 调用接口 (自动注入鉴权头)
owenc api POST /v1/orders/create -d '{"amount": 100}'
```
