...
pub fn run_build_script(git_dir: &str) {
    ...
    let cowen_build_id = std::env::var("COWEN_BUILD_ID").unwrap_or(build_id.clone());
    let cowen_build_time = std::env::var("COWEN_BUILD_TIME").unwrap_or(build_time.clone());
    ...
    println!("cargo:rustc-env=COWEN_BUILD_CLIENT_ID={}", builtin_client_id);
}
