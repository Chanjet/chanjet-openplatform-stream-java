...
if ! echo "$output" | grep -q "Global Configuration"; then
    fail_suite "Missing Global Configuration in --all text output"
fi

if ! echo "$output" | grep -q "Profile Configuration (p1.yaml)"; then
    fail_suite "Missing p1 in --all text output"
fi

if ! echo "$output" | grep -q "Profile Configuration (p2.yaml)"; then
