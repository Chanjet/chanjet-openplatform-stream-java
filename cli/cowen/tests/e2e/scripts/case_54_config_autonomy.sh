...
"$COWEN_BIN" config set dummy_array '[{"name":"p1","path":"/path1","type":"search"},{"name":"p2","path":"/path2","type":"search"}]'

echo "2. Test Identifier Locator (Set)"
"$COWEN_BIN" config set dummy_array.name:p1.path "/new/path1"
VAL=$("$COWEN_BIN" config get dummy_array.0.path | tr -d '"')
...
"$COWEN_BIN" config set dummy_array.+ '{"name":"p3","path":"/path3","type":"search"}'
LEN=$("$COWEN_BIN" config get dummy_array --format json | python3 -c "import sys, json; print(len(json.load(sys.stdin)))")
if [ "$LEN" -eq 3 ]; then
...
"$COWEN_BIN" config unset dummy_array.name:p2
# p1 is 0, p2 was 1, p3 was 2. After unset p2, p3 should become 1.
P3_PATH=$("$COWEN_BIN" config get dummy_array.1.path | tr -d '"')
...
"$COWEN_BIN" config set dummy_array.name:p1.name "p_new"
# locator name:p1 should now fail
if "$COWEN_BIN" config get dummy_array.name:p1 2>/dev/null; then
