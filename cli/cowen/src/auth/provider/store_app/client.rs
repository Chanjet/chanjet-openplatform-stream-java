...
pub(crate) async fn intercept_exchange(
    ...
    if let Some(identity) = token.extract_identity() {
        ...
        if !identity.user_id.is_empty() && identity.user_id != "0" {
            ...
            let key_pair = storage::get_user_token_key(app_key, &identity.org_id, &identity.user_id);
            vault
            ...
            if let Some(upc) = &token_resp.user_permanent_code {
                let upc_key = storage::get_user_upc_key(app_key, &identity.org_id, &identity.user_id);
            ...
            if let Some(opc) = &token_resp.org_permanent_code {
                let opc_key = storage::get_org_opc_key(app_key, &identity.org_id);
                vault.set(profile, &opc_key, opc).await?;
            }
            // Org-level token
            let key_pair = storage::get_org_token_key(app_key, &identity.org_id);
            vault
                .set(profile, &key_pair, &serde_json::to_string(&pair)?)
                .await?;
...
pub(crate) async fn get_app_access_token(
    ...
    // 1. 优先尝试从持久化池中获取
    if let Ok(token) = pool.get_app_access_token(&cfg.app_key).await {
...
pub(crate) async fn exchange_permanent_code_by_temp_code(
    ...
    let app_at = get_app_access_token(pool, http_sender, profile, cfg).await?.value;
    let url = format!(
    ...
    let opc_key = storage::get_org_opc_key(cfg.app_key.trim(), org_id);
    pool
        .as_vault()
...
pub(crate) async fn request_token(
    ...
    if let Some(identity) = token.extract_identity() {
        ...
        if let Some(upc) = token_resp.user_permanent_code {
            let upc_key = storage::get_user_upc_key(app_key, &identity.org_id, &identity.user_id);
            pool.as_vault().set(profile, &upc_key, &upc).await?;
        }
        if let Some(opc) = token_resp.org_permanent_code {
            let opc_key = storage::get_org_opc_key(app_key, &identity.org_id);
            pool.as_vault().set(profile, &opc_key, &opc).await?;
        }
        ...
        let _ = pool.as_vault().delete(profile, "oauth2_revoked").await;
