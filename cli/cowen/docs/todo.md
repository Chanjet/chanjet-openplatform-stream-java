...
- [ ] **Rust 客户端完善本地 DLQ (死信队列)**：
    - [ ] `cowen` 的 Stream Forwarder 守护进程，在收到服务端下发的 `EventFrame` 后，第一时间（在业务逻辑处理前）将其写入本地 SQLite 的 DLQ 中暂存。
    - [ ] 当业务处理成功并成功发送 200 ACK 后，从本地 DLQ 中抹除该消息。
    - [ ] 若处理慢导致断连，或因进程崩溃未能回复 ACK，重启后可从本地 DLQ 中恢复并重试投递。
