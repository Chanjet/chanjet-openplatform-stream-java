...
*   **[x] Task 1.1**: 更新工作区 `Cargo.toml`，注册新的 Crate 成员。
*   **[x] Task 1.2**: 使用 `cargo new --lib` 创建以下隔离的内部 Crate：
...
*   **验收标准 (Acceptance Criteria)**:
    1. [x] 工作区 `Cargo.toml` 正确注册 6 个新 Crate，并在物理目录结构中正确生成。
    2. [x] 执行 `cargo check --workspace` 编译无报错。
    3. [x] 全局既有测试用例 100% 顺利通过。

## Phase 2: 核心依赖去上帝化重构 (Decoupling & Splitting cowen-common)
*   **[x] Task 2.1: `cowen-infra` 底座创建与工具沉降**
    *   在 `crates/cowen-infra` 中编写基础工具代码。
    *   将原 `cowen-common` 中通用的加盐混淆 (obfs)、系统物理路径计算、低级时间转换等业务无关工具逻辑全部沉降至此 Crate。
*   **[x] Task 2.2: `cowen-common` 瘦身与极净化**
    *   清理 `cowen-common` 中的 `Cargo.toml` 依赖，移除 reqwest、tokio、redis 等一切厚重 I/O 库绑定。
    *   瘦身其源码，使其仅包含最通用稳定的核心数据模型与 SPI 契约 Trait。
*   **[x] Task 2.3: 全局依赖树梳理与编译修复**
    *   重构 `cowen-server`、`cowen-auth`、`cowen-store` 的 `Cargo.toml` 声明和源码中模块路径引入。
    *   确保依赖层次重定向后全工程一次性编译通过，物理上隔离循环依赖风险。
*   **验收标准 (Acceptance Criteria)**:
    1. [x] `cowen-common` 的 `Cargo.toml` 不含有任何网络及存储等重依赖，仅依赖极其稳定的核心模型库。
    2. [x] 依赖 `cowen-common` 的各个业务 Crate（如 `cowen-server` 等）重新定位底层结构引用，编译无警告和报错。
    3. [x] 运行 `cargo test --workspace` 完美全绿，原有所有测试用例 100% 通过。

## Phase 3: 配置热重载 (cowen-config)
*   **[x] Task 3.1: `cowen-config` 核心实现**
    *   引入 `notify` 和 `tokio` 依赖。
    *   实现基于 `notify` 的配置文件变更监听器和 `SIGHUP` 信号监听。
    *   利用 `tokio::sync::watch` 对外暴露配置订阅能力。
*   **[x] Task 3.2: Daemon 集成与日志级别动态调整**
    *   在 `cowen-server` (Daemon) 中引入 `cowen-config`。
    *   修改 `ProxyServer` 等长连接后台任务，使其订阅配置更新。
    *   对接 `tracing-subscriber` 的 Reload 句柄，随配置更新动态改变日志级别。
*   **[x] Task 3.3 (E2E)**: 实现 `tests/e2e/scripts/case_45_config_hot_reload.sh` 验证热重载不断流及日志级别的变化。
*   **验收标准 (Acceptance Criteria)**:
    1. [x] 新增针对不可热载字段（如 `db_url` 等物理模型关键值）变动拦截的 TDD 单元测试并 100% 通过。
    2. [x] 自动化 E2E 脚本 `case_45_config_hot_reload.sh` 运行成功：修改 `app.yaml` 日志级别后进程不断流且 PID 保持不变，新的日志级别在 Daemon 运行中即时生效。

## Phase 4: 监控与健康 API (cowen-monitor)
*   **[x] Task 4.1: `cowen-monitor` 服务搭建**
    *   引入 `prometheus` 和 `axum`。
    *   实现独立运行在本地端口的 HTTP 服务，暴露 `/health` 和 `/metrics`。
*   **[x] Task 4.2: 宏定义与无侵入埋点**
    *   在 `cowen-monitor` 导出打点宏 (如 `counter!()`)。
    *   在 `cowen-server` (Proxy 处理层等) 引入宏进行无侵入式指标统计。
*   **[x] Task 4.3 (E2E)**: 实现 `tests/e2e/scripts/case_46_metrics_health.sh` 验证端点连通性及指标累加逻辑。
*   **验收标准 (Acceptance Criteria)**:
    1. [x] 新增针对监控注册中心和指标增加的 TDD 单元测试并 100% 通过。
    2. [x] 自动化 E2E 脚本 `case_46_metrics_health.sh` 运行成功：监控端口独立运行在 `127.0.0.1`，通过请求代理转发后，`/metrics` 能准确采集累加指标，`/health` 输出规范的 UP 状态。

## Phase 5: 环境自检工具 (cowen-doctor)
*   **[x] Task 5.1: `cowen-doctor` SPI 与调度台**
...
*   **[x] Task 5.4 (E2E)**: 实现 `tests/e2e/scripts/case_47_system_doctor.sh` 验证故意配错时能输出预期的 ERROR 和建议。
*   **验收标准 (Acceptance Criteria)**:
    1. [x] 新增诊断器调度超时的 TDD 单元测试并 100% 通过。
    2. [x] 自动化 E2E 脚本 `case_47_system_doctor.sh` 运行成功：当手动损坏数据库配置或断开网络时，运行 `cowen system doctor` 能准确抛出 `[ERROR]` 并输出对应的推荐修复建议（`Recommendation`）。

## Phase 6: API 搜索插件化 (cowen-search)
*   **[x] Task 6.1: `cowen-search` SPI 与内置实现**
    *   定义 `SearchProvider` Trait。
    *   实现内置基于字符串匹配的 `StringMatchProvider`。
*   **[x] Task 6.2: `cowen-search-embedding` 动态库化与 C ABI**
    *   将插件逻辑封装为 `cdylib`，导出符合 C ABI 规范的 `v1_init` 和 `v1_free` 接口。
*   **[x] Task 6.3: 插件自动发现与动态加载机制**
    *   引入 `libloading` 实现运行时加载。
    *   实现插件目录的 `discover_plugins` 扫描逻辑，支持跨平台扩展名（`.dylib`, `.so`, `.dll`）。
    *   引入插件元数据解析，支持按 `type`/`capability` 分类管理。
*   **[x] Task 6.4: 配置映射与显式启用机制**
    *   实现基于 YAML 映射表的显式插件启用机制，支持通过 `enabled: ["name1", "name2"]` 同时组合多种能力插件。
*   **[x] Task 6.5: 优雅降级与 E2E 验证**
    *   实现加载失败时的 Fallback 降级。
    *   编写 `tests/e2e/scripts/case_48_search_plugin.sh` 验证自动发现、显式启用及能力组合效果。
*   **验收标准 (Acceptance Criteria)**:
    1. [x] 单元测试覆盖自动发现目录、插件 metadata 解析及配置加载逻辑。
    2. [x] 自动化 E2E 脚本 `case_48_search_plugin.sh` 验证：插件自动发现感知、多种能力插件的显式组合启用、插件不存在时的优雅退化。

## Phase 7: 健壮性与进程自愈增强 (Robustness & Self-Healing Enhancements)
*   **[x] Task 7.1: DLQ 存储异常 Panic 防护实现**
    *   重构 `Forwarder::new` 签名，返回 `Result<Self, CowenError>`。
    *   在 `bridge.rs` (Daemon 启动) 和 `cmd/dlq.rs` (重试入口) 中捕获存储初始化异常，优雅输出日志并平滑退场，杜绝进程崩溃 Panic。
    *   编写单元测试模拟连接损坏场景进行验证。
*   **[x] Task 7.2: 智能动态 Token 检查与自适应刷新实现**
...
*   **[x] Task 7.3 (E2E)**: 实现 `tests/e2e/scripts/case_49_robustness_check.sh` 自动验证死信队列损坏时的安全退出和 Token 定时调度的自适应演进状态。
*   **验收标准 (Acceptance Criteria)**:
    1. [x] 新增死信初始化坏库测试用例、Token 定时休眠计算公式用例（含上限 3600s、下限 30s 与随机 Jitter 偏移）的 TDD 单元测试并 100% 全绿通过。
    2. [x] 自动化 E2E 脚本 `case_49_robustness_check.sh` 运行成功：在坏库场景下进程平滑输出致命级日志并安全退出（退出码为 1），主进程不崩溃；在大周期测试下，Token 检测线程严格按照寿命公式及波动范围进行自适应休眠。

## Phase 8: 文档与发布 (Release & Docs)
*   **[x] Task 8.1**: 更新 `docs/COMMANDS.md` 补充新命令和配置项。
*   **[x] Task 8.2**: 更新安装脚本/Makefile，处理 `libcowen_search_embedding` 的可选分发。
*   **验收标准 (Acceptance Criteria)**:
    1. [x] 编译构建生成的发布压缩包大小满足预定指标，且动态库打包逻辑可分离。
    2. [x] 全工程及所有测试用例、E2E 测试脚本 100% 顺利通过。