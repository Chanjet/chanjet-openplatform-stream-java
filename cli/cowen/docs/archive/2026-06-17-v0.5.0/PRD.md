...
> [!IMPORTANT]
> **适用范围声明**：当前的 Identity-Aware Gateway（反向代理与入口身份拦截）特性**仅对 `store-app` (应用商店应用) 模式开放**。由于鉴权生命周期与协议层的根本差异，`self-built` (自建应用) 与纯粹的 `oauth2` 模式下不可启用此网关能力。如果强行在非 `store-app` 模式下配置 `gateway` 节点，Cowen 进程将会阻断并拒绝启动。
