...
> [!IMPORTANT]
> **结合 Stream Gateway 的定位与三方生态发展的演进路线：**
> 
> 1.  **第一阶段（当前）**：
>     **立即停用并淘汰 Native dylib 作为三方挂载方案**。将其降级为仅官方内置核心组件的硬编码加载机制。
> 2.  **第二阶段（快速扩展与 Agent 对接）**：
>     **全面采用“RPC/Stdio 子进程挂载（Sidecar 模式）”**。这是让 Agent 开发者以最舒服的 **Python** 和原生的 **MCP 规范**快速在本地实现插件的唯一破局路径。
> 3.  **第三阶段（轻量化数据清洗）**：
>     对于高吞吐、零依赖、追求绝对内存安全的数据管道过滤组件，在未来可以引入 **WebAssembly (Wasm) 作为进程内轻量沙箱**，实现混合架构（重 AI 计算走子进程，轻量级路由过滤走 Wasm）。
