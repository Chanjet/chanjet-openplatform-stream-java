...
### 3. 极简网络面收敛 (Minimal Network Surface)
- **机制**: 在不需要为传统 HTTP 客户端提供本地代理的场景下，支持通过设置 `proxy_enabled = false` (或启动时添加 `--no-proxy` 参数) 彻底关闭 `proxy_port` 及所有本地 HTTP 代理路由。
- **安全收益**: 关闭代理后，系统不再对外暴露任何本地 HTTP 端口，所有插件交互强制退化为单一的 **Stdio JSON-RPC** 通道 (`transport: "stdio"`)，彻底切断了任何通过 HTTP 越权探测宿主内部接口的可能性，实现最严苛的进程级安全隔离。
