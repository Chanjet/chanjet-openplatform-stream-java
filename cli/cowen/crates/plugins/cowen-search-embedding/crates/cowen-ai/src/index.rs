...
impl SearchIndex {
    ...
    pub fn ensure_assets(base_dir: &std::path::Path) -> anyhow::Result<()> {
        ...
        // Use files generated by build script via env vars
        let model_path_env = env!("MODEL_ONNX");
        let data_path_env = env!("MODEL_ONNX_DATA");
        Self::ensure_asset(&model_path, &fs::read(model_path_env)?)?;
        Self::ensure_asset(&data_path, &fs::read(data_path_env)?)?;
        Self::ensure_asset(
            &tokenizer_path,
            include_bytes!("../assets/models/tokenizer.json"),
        )?;
