...
async fn perform_handshake() {
    if let Ok(mut daemon_client) = client::get_daemon_grpc_client().await {
        ...
        req.required_capabilities
            .insert("native.api.registry".to_string(), "1.0.0".to_string());
...
async fn run_mcp_server(profile: String) -> Result<(), Box<dyn std::error::Error>> {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        ...
        Some(Commands::Server) => {
            run_mcp_server(cli.profile).await?;
        }
        None => {
