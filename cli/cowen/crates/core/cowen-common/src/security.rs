...
pub fn get_machine_fingerprint() -> CowenResult<String> {
    match cowen_sys::get_sys_fingerprint().get_machine_id() {
        Ok(uuid) => Ok(uuid),
        Err(e) => Err(CowenError::Security(format!(
            "Failed to retrieve machine fingerprint: {}",
            e
        ))),
    }
}
...
pub mod ssrf {
    ...
    fn resolve_host_ip(host: &str) -> CowenResult<IpAddr> {
        match host.parse() {
            Ok(addr) => Ok(addr),
            Err(_) => {
                if host == "localhost" {
                    Ok(std::net::Ipv4Addr::new(127, 0, 0, 1).into())
                } else {
    ...
    fn check_flexible_whitelist(ip: &IpAddr, whitelist: &[String]) -> CowenResult<()> {
        if ip.is_loopback() {
            return Ok(());
        }
    ...
    pub fn validate_ssrf(
        ...
        let url = url::Url::parse(url_str)
            .map_err(|e| CowenError::api(format!("Invalid webhook URL: {}", e)))?;
        ...
        if matches!(level, SecurityLevel::Strict) {
            if !ip.is_loopback() {
                return Err(CowenError::api(
                    "SSRF Violation: Only loopback addresses allowed in Strict mode.",
                ));
            }
            return Ok(());
        }
        ...
        if matches!(level, SecurityLevel::Flexible) {
            check_flexible_whitelist(&ip, whitelist)?;
        }
