...
use crate::grpc::proto::{self as grpc_proto,
...
impl DaemonClient {
    ...
    pub async fn ensure_daemon(&self) -> Result<(Channel, AuthInterceptor)> {
        if let Ok((channel, interceptor)) = self.connect_to_daemon().await {
            ...
            loop {
                let mut test_client = NativeWorkerServiceClient::with_interceptor(channel.clone(), interceptor.clone());
        ...
        for _ in 0..30 {
            ...
            if let Ok((channel, interceptor)) = self.connect_to_daemon().await {
                let mut client = NativeWorkerServiceClient::with_interceptor(channel.clone(), interceptor.clone());
    ...
    async fn build_native_audit_client(&self) -> Result<NativeAuditServiceClient<tonic::codegen::InterceptedService<Channel, AuthInterceptor>>> {
        let (channel, interceptor) = self.ensure_daemon().await?;
        Ok(NativeAuditServiceClient::with_interceptor(channel, interceptor))
    }
    async fn build_native_auth_client(&self) -> Result<NativeAuthServiceClient<tonic::codegen::InterceptedService<Channel, AuthInterceptor>>> {
        let (channel, interceptor) = self.ensure_daemon().await?;
        Ok(NativeAuthServiceClient::with_interceptor(channel, interceptor))
    }
    async fn build_native_config_client(&self) -> Result<NativeConfigServiceClient<tonic::codegen::InterceptedService<Channel, AuthInterceptor>>> {
        let (channel, interceptor) = self.ensure_daemon().await?;
        Ok(NativeConfigServiceClient::with_interceptor(channel, interceptor))
    }
    async fn build_native_dlq_client(&self) -> Result<NativeDlqServiceClient<tonic::codegen::InterceptedService<Channel, AuthInterceptor>>> {
        let (channel, interceptor) = self.ensure_daemon().await?;
        Ok(NativeDlqServiceClient::with_interceptor(channel, interceptor))
    }
    async fn build_native_system_client(&self) -> Result<NativeSystemServiceClient<tonic::codegen::InterceptedService<Channel, AuthInterceptor>>> {
        let (channel, interceptor) = self.ensure_daemon().await?;
        Ok(NativeSystemServiceClient::with_interceptor(channel, interceptor))
    }
    
    async fn build_api_registry_client(&self) -> Result<ApiRegistryServiceClient<tonic::codegen::InterceptedService<Channel, AuthInterceptor>>> {
        let (channel, interceptor) = self.ensure_daemon().await?;
        Ok(ApiRegistryServiceClient::with_interceptor(channel, interceptor))
    }
    async fn build_native_worker_client(&self) -> Result<NativeWorkerServiceClient<tonic::codegen::InterceptedService<Channel, AuthInterceptor>>> {
        let (channel, interceptor) = self.ensure_daemon().await?;
    ...
    pub async fn stop_daemon(&self, profile: &str) -> Result<DaemonResponse> {
        let mut client = match self.connect_to_daemon().await {
            Ok((channel, interceptor)) => NativeWorkerServiceClient::with_interceptor(channel, interceptor),
