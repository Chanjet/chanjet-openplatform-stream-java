...
pub fn mask_sensitive_json(input: &str) -> String {
    ...
    let keys = [
        "\"accessToken\"", "\"access_token\"", "\"orgAccessToken\"", "\"userAccessToken\"",
        "\"refreshToken\"", "\"refresh_token\"", "\"appSecret\"", "\"app_secret\"",
        "\"certificate\"", "\"appTicket\"", "\"app_ticket\"", "\"encryptKey\"", "\"encrypt_key\"",
        "\"permanentAuthCode\"", "\"userPermanentCode\"", "\"user_auth_permanent_code\"",
        "\"tempAuthCode\"",
    ];
...
pub fn mask_url_query(url: &str) -> String {
    ...
    let keys = [
        "accessToken=", "access_token=", "orgAccessToken=", "userAccessToken=",
        "refreshToken=", "refresh_token=", "token=", "openToken=",
        "appSecret=", "app_secret=", "appTicket=", "app_ticket=",
        "encryptKey=", "encrypt_key=", "permanentAuthCode=",
        "userPermanentCode=", "tempAuthCode=",
    ];
    ...
    for key in keys {
        ...
        for search_key_lower in &[qs_lower, am_lower] {
            ...
            while let Some(idx) = output[start_idx..].to_lowercase().find(search_key_lower) {
                ...
                let val_end = output[val_start..].find('&').map(|i| val_start + i).unwrap_or(output.len());
                let secret = &output[val_start..val_end];
                let masked = mask_string(secret);
