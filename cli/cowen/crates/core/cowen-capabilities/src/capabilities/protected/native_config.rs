...
pub trait NativeConfigCapability: Send + Sync {
    ...
    async fn get_app_ticket(&self, _claims: Option<&cowen_common::jwt::IpcClaims>, app_key: &str) -> Result<Option<cowen_common::models::Ticket>, CowenError>;
    async fn get_app_secret(&self, _claims: Option<&cowen_common::jwt::IpcClaims>, profile: &str) -> Result<String, CowenError>;
