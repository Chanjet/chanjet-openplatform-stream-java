...
pub trait NativeSystemCapability: Send + Sync {
    type TunnelPluginStream: tokio_stream::Stream<
            Item = Result<cowen_common::grpc::proto::TunnelPluginResponse, CowenError>,
        > + Send
        + 'static;
    ...
    async fn doctor(
        &self,
        claims: Option<&cowen_common::jwt::IpcClaims>,
        req: DomainDoctorRequest,
    ) -> Result<DomainDoctorResponse, CowenError>;
    async fn store_status(
        &self,
        claims: Option<&cowen_common::jwt::IpcClaims>,
        req: DomainStoreStatusRequest,
    ) -> Result<DomainStoreStatusResponse, CowenError>;
    async fn system_status(
        &self,
        claims: Option<&cowen_common::jwt::IpcClaims>,
        req: DomainSystemStatusRequest,
    ) -> Result<DomainSystemStatusResponse, CowenError>;
...
impl DefaultSystem {
    ...
    async fn collect_profile_status(
        &self,
        prof: &str,
        app_config: &cowen_common::config::AppConfig,
        ...
        if !self.cfg_mgr.exists(prof).await
            && config.app_key.is_empty()
        ...
        let daemon_entry = cowen_common::status::collect_daemon_status(
            &ctx,
            "Daemon",
            "Tips",
            supports_webhooks,
        ...
        if config.proxy_enabled {
            let status_file =
                cowen_common::config::get_app_dir().join(format!("{}_status.json", prof));
            let mut active_proxy_port = None;
            if let Ok(content) = std::fs::read_to_string(status_file) {
                if let Ok(json) = serde_json::from_str::<serde_json::Value>(&content) {
                    active_proxy_port = json
                        .get("proxy_port")
                        .and_then(|v| v.as_u64())
                        .map(|p| p as u16);
                }
    ...
    fn validate_plugin_request(
        &self,
        first_msg: &cowen_common::grpc::proto::TunnelPluginRequest,
    ) -> Result<(String, std::path::PathBuf, String), CowenError> {
        let plugin_name = first_msg.plugin_name.clone().ok_or_else(|| {
            CowenError::Validation("First message must contain plugin_name".to_string())
        })?;
        ...
        let jwt_secret_vec = cowen_common::jwt::get_global_daemon_secret()
            .cloned()
            .unwrap_or_default();
        ...
        Ok((plugin_name, expected_path, bridge_token))
    }

    fn spawn_tunnel_io_tasks(
        &self,
        first_msg: cowen_common::grpc::proto::TunnelPluginRequest,
        stream: tonic::Streaming<cowen_common::grpc::proto::TunnelPluginRequest>,
        ...
        tx: tokio::sync::mpsc::Sender<
            Result<cowen_common::grpc::proto::TunnelPluginResponse, CowenError>,
        >,
    ) {
        ...
        tokio::spawn(async move {
            ...
            while let Ok(Some(msg)) = stream.message().await {
                if let Some(payload) = msg.stdin_payload {
                    ...
                    if tokio::io::AsyncWriteExt::write_all(&mut stdin, &payload)
                        .await
                        .is_err()
                    {
                        break;
        ...
        tokio::spawn(async move {
            ...
            loop {
                match tokio::io::AsyncReadExt::read(&mut stdout, &mut buf).await {
                    ...
                    Ok(n) => {
                        if tx_out
                            .send(Ok(cowen_common::grpc::proto::TunnelPluginResponse {
...
impl NativeSystemCapability for DefaultSystem {
    type TunnelPluginStream = std::pin::Pin<
        Box<
    ...
    async fn store_status(
        &self,
        _claims: Option<&cowen_common::jwt::IpcClaims>,
        _req: DomainStoreStatusRequest,
    ) -> Result<DomainStoreStatusResponse, CowenError> {
        let app_config: cowen_common::config::AppConfig =
    ...
    async fn system_status(
        ...
        if !profiles.is_empty() {
            for prof in profiles {
                ...
                if let Some(entry_val) = self.collect_profile_status(&prof, &app_config).await {
    ...
    async fn system_reset(
        ...
        if dry_run {
            ...
            // OCP: Clear profile from memory cache and trigger vault deletion via ConfigManager
            {
                ...
                } else if let Ok(profiles) = cfg_mgr.list_profiles().await {
                    for p in profiles {
                        if let Err(e) = cfg_mgr.delete(&p).await {
                            errors.push(format!("ConfigManager Reset: {}", e));
                        }
            ...
            if errors.is_empty() {
                ...
                Ok(DomainSystemResetResponse {
                    success: false,
                    message: format!("Errors occurred: {}", errors.join(", ")),
    ...
    async fn tunnel_plugin(
        ...
        let (_plugin_name, expected_path, bridge_token) =
            self.validate_plugin_request(&first_msg)?;

        let port_str = self.ipc_port.to_string();
        let profile = first_msg
            .envs
            .get("COWEN_PROFILE")
            .cloned()
        ...
        let mut child = match cmd.spawn() {
            Ok(c) => c,
            Err(e) => {
                return Err(CowenError::Internal(format!(
        ...
        self.spawn_tunnel_io_tasks(first_msg, stream, stdin, stdout, stderr, tx);
