...
impl DefaultSystem {
    ...
    fn validate_plugin_request(
        ...
        let plugin_name = first_msg.plugin_name.clone().ok_or_else(|| CowenError::Validation("First message must contain plugin_name".to_string()))?;
    ...
    fn spawn_tunnel_io_tasks(
        ...
        tx: tokio::sync::mpsc::Sender<Result<cowen_common::grpc::proto::TunnelPluginResponse, CowenError>>,
    ) {
...
impl NativeSystemCapability for DefaultSystem {
    ...
    async fn tunnel_plugin(
        ...
        let (_plugin_name, expected_path, bridge_token) = self.validate_plugin_request(&first_msg)?;
