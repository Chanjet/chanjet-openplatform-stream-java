...
pub trait NativeDlqCapability: Send + Sync {
    async fn dlq_list(
        ...
        req: DomainDlqListRequest,
    ) -> Result<DomainDlqListResponse, CowenError>;
    async fn dlq_view(
        &self,
        claims: Option<&cowen_common::jwt::IpcClaims>,
        req: DomainDlqViewRequest,
    ) -> Result<DomainDlqViewResponse, CowenError>;
    async fn dlq_retry(
        &self,
        claims: Option<&cowen_common::jwt::IpcClaims>,
        req: DomainDlqRetryRequest,
    ) -> Result<DomainDlqRetryResponse, CowenError>;
    async fn dlq_purge(
        &self,
        claims: Option<&cowen_common::jwt::IpcClaims>,
        req: DomainDlqPurgeRequest,
    ) -> Result<DomainDlqPurgeResponse, CowenError>;
}
...
impl NativeDlqCapability for DefaultDlq {
    ...
    async fn dlq_list(
        ...
        match self
            .vault
            .list_dlq(&req.profile, req.page_size as usize)
            .await
        {
            Ok(msgs) => Ok(DomainDlqListResponse {
                json: serde_json::to_string(&msgs).unwrap_or_default(),
                error_message: None,
            }),
            Err(e) => Ok(DomainDlqListResponse {
                json: "".to_string(),
    ...
    async fn dlq_view(
        ...
        match self.vault.get_dlq_by_id(id_i64).await {
            Ok(Some(msg)) => Ok(DomainDlqViewResponse {
                json: serde_json::to_string(&msg).unwrap_or_default(),
                error_message: None,
            }),
            Ok(None) => Ok(DomainDlqViewResponse {
                json: "".to_string(),
                error_message: Some("Not found".to_string()),
            }),
            Err(e) => Ok(DomainDlqViewResponse {
                json: "".to_string(),
                error_message: Some(e.to_string()),
            }),
        }
    ...
    async fn dlq_retry(
        ...
        match crate::internal::forwarder::Forwarder::new(
            &req.profile,
            config,
            &app_cfg,
            self.vault.clone(),
        ) {
            Ok(forwarder) => match forwarder.retry_message(id_i64).await {
                Ok(_) => Ok(DomainDlqRetryResponse {
                    success: true,
                    message: "Retried".to_string(),
                    error_message: None,
                }),
                Err(e) => Ok(DomainDlqRetryResponse {
                    success: false,
                    message: "".to_string(),
                    error_message: Some(e.to_string()),
                }),
    ...
    async fn dlq_purge(
        ...
        match self.vault.list_all_dlq(&req.profile).await {
            Ok(msgs) => {
                ...
                for m in msgs {
                    if let Some(id) = m.id {
                        if self.vault.delete_dlq_by_id(id).await.is_ok() {
                            count += 1;
                        }
                    }
                }
                Ok(DomainDlqPurgeResponse {
                    success: true,
                    message: format!("Purged {} messages", count),
                    error_message: None,
                })
            }
            Err(e) => Ok(DomainDlqPurgeResponse {
                success: false,
                message: "".to_string(),
                error_message: Some(e.to_string()),
            }),
