...
async fn handle_tools_list(req: JsonRpcRequest, app_state: &AppState) -> JsonRpcResponse {
    let mut tools = vec![
        json!({
            "name": "cowen_api_list",
...
async fn handle_tools_call(req: JsonRpcRequest, app_state: &AppState) -> (JsonRpcResponse, bool) {
