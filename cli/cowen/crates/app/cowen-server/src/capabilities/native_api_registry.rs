...
pub trait NativeApiRegistryCapability: Send + Sync {
    async fn api_spec(&self, claims: Option<&cowen_common::jwt::IpcClaims>, req: DomainApiSpecRequest) -> Result<DomainApiSpecResponse, CowenError>;
    async fn call_api(&self, claims: Option<&cowen_common::jwt::IpcClaims>, req: DomainCallApiRequest) -> Result<DomainCallApiResponse, CowenError>;
    async fn api_list(&self, claims: Option<&cowen_common::jwt::IpcClaims>, req: DomainApiListRequest) -> Result<DomainApiListResponse, CowenError>;
}

pub struct DefaultApiRegistry {
    vault: Arc<dyn Vault>,
    cfg_mgr: ConfigManager,
    native_search: Arc<dyn crate::capabilities::native_search::NativeSearchCapability>,
}

impl DefaultApiRegistry {
    pub fn new(vault: Arc<dyn Vault>, cfg_mgr: ConfigManager, native_search: Arc<dyn crate::capabilities::native_search::NativeSearchCapability>) -> Self {
...
impl NativeApiRegistryCapability for DefaultApiRegistry {
    #[rbac(action = "read")]
    async fn api_spec(&self, _claims: Option<&cowen_common::jwt::IpcClaims>, req: DomainApiSpecRequest) -> Result<DomainApiSpecResponse, CowenError> {
                ...
                match serde_json::from_str::<serde_json::Value>(&spec_str) {
                    Ok(spec) => {
                        ...
                        if let Some(op) = spec.get("paths").and_then(|p| p.as_object()).and_then(|p| p.get(&req.path)).and_then(|p| p.get(&method_lower)) {
                            ...
                            Ok(DomainApiSpecResponse { json, error_message: None })
                        } else {
                            Ok(DomainApiSpecResponse { json: "".to_string(), error_message: Some("Operation not found".to_string()) })
                        }
                    },
                    Err(_) => Ok(DomainApiSpecResponse { json: spec_str, error_message: None }),
                }
            }
    ...
    #[rbac(action = "search")]
    async fn api_list(&self, _claims: Option<&cowen_common::jwt::IpcClaims>, req: DomainApiListRequest) -> Result<DomainApiListResponse, CowenError> {
        ...
        match auth_cli.get_openapi_spec(&req.profile, &config, req.refresh).await {
            Ok(spec) => {
                let mut ops = crate::daemon::grpc_capabilities::openapi_parser::OpenApiParser::parse_operations(&spec);
                ...
                Ok(DomainApiListResponse { total, json, plugin_used: used_plugin_name, error_message: None })
            },
            Err(e) => Err(CowenError::Internal(e.to_string()))
        }
    }
