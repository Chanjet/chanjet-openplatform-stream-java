...
# One-liner config for all pods (Self-Built Mode)
export COWEN_APP_MODE="self-built"
export COWEN_APP_KEY="AK_SB_STRESS"
export COWEN_APP_SECRET="AS_SB_STRESS"
export COWEN_CERTIFICATE="CERT_SB_STRESS"
...
for i in {1..4}; do
    ...
    COWEN_HOME="$POD_HOME" "$COWEN_BIN" daemon start --foreground > "$POD_HOME/daemon.log" 2>&1 < /dev/null &
    PIDS+=($!)
done
...
echo -e "\n${GREEN}🎊 Case 30 Passed! (Self-Built Sidecar Scaling Resilience Verified)${NC}"
