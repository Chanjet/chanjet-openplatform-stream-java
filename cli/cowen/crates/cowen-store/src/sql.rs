...
impl Store for SqlStore {
    // --- Config ---
    async fn get_config(&self, profile: &str, key: &str) -> CowenResult<String> { self.driver.get_config(profile, key).await }
    async fn get_config_metadata(&self, profile: &str, key: &str) -> CowenResult<(u64, i64)> { self.driver.get_config_metadata(profile, key).await }
    async fn get_config_full(&self, profile: &str, key: &str) -> CowenResult<Item> { self.driver.get_config_full(profile, key).await }
    async fn set_config(&self, profile: &str, key: &str, value: &str) -> CowenResult<()> { self.driver.set_config(profile, key, value).await }
    async fn set_config_conditional(&self, profile: &str, key: &str, value: &str, ev: u64) -> CowenResult<()> { self.driver.set_config_conditional(profile, key, value, ev).await }
    async fn list_configs(&self, profile: &str) -> CowenResult<Vec<String>> { self.driver.list_configs(profile).await }
    async fn delete_config(&self, profile: &str, key: &str) -> CowenResult<()> { self.driver.delete_config(profile, key).await }

    // --- Secret ---
    async fn get_secret(&self, profile: &str, key: &str) -> CowenResult<String> { self.driver.get_secret(profile, key).await }
    async fn set_secret(&self, profile: &str, key: &str, value: &str) -> CowenResult<()> { self.driver.set_secret(profile, key, value).await }
    async fn delete_secret(&self, profile: &str, key: &str) -> CowenResult<()> { self.driver.delete_secret(profile, key).await }
    async fn list_secrets(&self, profile: &str) -> CowenResult<Vec<String>> { self.driver.list_secrets(profile).await }

    // --- Token ---
    async fn get_access_token(&self, profile: &str) -> CowenResult<cowen_common::models::Token> { self.driver.get_access_token(profile).await }
    async fn save_access_token(&self, profile: &str, token: cowen_common::models::Token) -> CowenResult<()> { self.driver.save_access_token(profile, token).await }
    async fn delete_access_token(&self, profile: &str) -> CowenResult<()> { self.driver.delete_access_token(profile).await }
    ...
    // --- Legacy Support ---
    async fn get_token(&self, profile: &str, key: &str) -> CowenResult<String> { self.driver.get_token(profile, key).await }
    async fn set_token(&self, profile: &str, key: &str, value: &str, exp: u64) -> CowenResult<()> { self.driver.set_token(profile, key, value, exp).await }
    async fn delete_token(&self, profile: &str, key: &str) -> CowenResult<()> { self.driver.delete_token(profile, key).await }
    async fn list_tokens(&self, profile: &str) -> CowenResult<Vec<String>> { self.driver.list_tokens(profile).await }

    // --- Audit ---
    async fn save_audit(&self, entry: &AuditEntry) -> CowenResult<()> { self.driver.save_audit(entry).await }
    async fn list_audit(&self, profile: &str, limit: usize) -> CowenResult<Vec<AuditEntry>> { self.driver.list_audit(profile, limit).await }

    // --- DLQ ---
    async fn push_dlq(&self, msg: &DlqMessage) -> CowenResult<()> { self.driver.push_dlq(msg).await }
    async fn pop_dlq(&self, profile: &str, topic: &str) -> CowenResult<Option<DlqMessage>> { self.driver.pop_dlq(profile, topic).await }
    async fn list_dlq(&self, profile: &str, limit: usize) -> CowenResult<Vec<DlqMessage>> { self.driver.list_dlq(profile, limit).await }
    async fn list_all_dlq(&self, profile: &str) -> CowenResult<Vec<DlqMessage>> { self.driver.list_all_dlq(profile).await }

    // --- Management ---
    async fn clear_profile(&self, profile: &str) -> CowenResult<()> { self.driver.clear_profile(profile).await }
