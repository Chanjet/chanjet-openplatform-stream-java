...
impl Store for SqlStore {
    ...
    // --- Token ---
    async fn get_access_token(&self, profile: &str) -> Result<cowen_common::models::Token> { self.driver.get_access_token(profile).await }
    async fn save_access_token(&self, profile: &str, token: cowen_common::models::Token) -> Result<()> { self.driver.save_access_token(profile, token).await }
    async fn delete_access_token(&self, profile: &str) -> Result<()> { self.driver.delete_access_token(profile).await }
    async fn get_app_access_token(&self, app_key: &str) -> Result<cowen_common::models::Token> { self.driver.get_app_access_token(app_key).await }
    async fn save_app_access_token(&self, app_key: &str, token: cowen_common::models::Token) -> Result<()> { self.driver.save_app_access_token(app_key, token).await }

    // --- Ticket ---
    async fn get_app_ticket(&self, app_key: &str) -> Result<cowen_common::models::Ticket> { self.driver.get_app_ticket(app_key).await }
    async fn save_app_ticket(&self, app_key: &str, ticket: cowen_common::models::Ticket) -> Result<()> { self.driver.save_app_ticket(app_key, ticket).await }
    ...
    async fn raw_del(&self, key: &str) -> Result<()> { self.driver.raw_del(key).await }
