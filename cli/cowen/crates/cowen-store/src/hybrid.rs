...
impl Store for HybridStore {
    async fn get_config(&self, p: &str, k: &str) -> CowenResult<String> {
        if let Ok(v) = self.cache.get_config(p, k).await { return Ok(v); }
    ...
    async fn get_access_token(&self, p: &str) -> CowenResult<Token> {
        if let Ok(v) = self.cache.get_access_token(p).await { return Ok(v); }
    ...
    async fn get_refresh_token(&self, p: &str) -> CowenResult<Token> {
        if let Ok(v) = self.cache.get_refresh_token(p).await { return Ok(v); }
    ...
    async fn get_app_access_token(&self, app_key: &str) -> CowenResult<Token> {
        if let Ok(v) = self.cache.get_app_access_token(app_key).await { return Ok(v); }
    ...
    async fn get_app_ticket(&self, app_key: &str) -> CowenResult<Ticket> {
        if let Ok(v) = self.cache.get_app_ticket(app_key).await { return Ok(v); }
    ...
    async fn get_org_permanent_code(&self, app_key: &str, org_id: &str) -> CowenResult<String> {
        if let Ok(v) = self.cache.get_org_permanent_code(app_key, org_id).await { return Ok(v); }
    ...
    async fn get_user_permanent_code(&self, app_key: &str, org_id: &str, user_id: &str) -> CowenResult<String> {
        if let Ok(v) = self.cache.get_user_permanent_code(app_key, org_id, user_id).await { return Ok(v); }
    ...
    async fn get_token(&self, p: &str, k: &str) -> CowenResult<String> {
        if let Ok(v) = self.cache.get_token(p, k).await { return Ok(v); }
    ...
    async fn list_tokens(&self, p: &str) -> CowenResult<Vec<String>> { self.persistence.list_tokens(p).await }

    async fn save_audit(&self, e: &AuditEntry) -> CowenResult<()> { self.persistence.save_audit(e).await }
    async fn list_audit(&self, p: &str, l: usize) -> CowenResult<Vec<AuditEntry>> { self.persistence.list_audit(p, l).await }
    async fn push_dlq(&self, m: &DlqMessage) -> CowenResult<()> { self.persistence.push_dlq(m).await }
    async fn pop_dlq(&self, p: &str, t: &str) -> CowenResult<Option<DlqMessage>> { self.persistence.pop_dlq(p, t).await }
    async fn list_dlq(&self, p: &str, l: usize) -> CowenResult<Vec<DlqMessage>> { self.persistence.list_dlq(p, l).await }
    async fn list_all_dlq(&self, p: &str) -> CowenResult<Vec<DlqMessage>> { self.persistence.list_all_dlq(p).await }
    async fn get_dlq_by_id(&self, id: i64) -> CowenResult<Option<DlqMessage>> { self.persistence.get_dlq_by_id(id).await }
    ...
    async fn list_all_profiles(&self) -> CowenResult<Vec<String>> { self.persistence.list_all_profiles().await }
    ...
    fn description(&self) -> String {
        format!("Hybrid (Cache: {}, Persistence: {})", self.cache.name(), self.persistence.name())
    }
}
