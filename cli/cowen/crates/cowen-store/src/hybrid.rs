...
pub struct HybridStore {
    ...
    persistence: Arc<dyn cowen_common::store::Store>,
}

impl HybridStore {
    pub fn new(cache: Arc<dyn cowen_common::store::Store>, persistence: Arc<dyn cowen_common::store::Store>) -> Self {
        Self { cache, persistence }
    }
}

#[async_trait]
impl cowen_common::store::Store for HybridStore {
    ...
    async fn raw_del(&self, key: &str) -> CowenResult<()> { self.cache.raw_del(key).await?; self.persistence.raw_del(key).await }
