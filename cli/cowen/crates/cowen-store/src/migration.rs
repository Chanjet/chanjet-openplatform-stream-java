...
impl StoreMigrator {
    ...
    pub async fn migrate(&self, cfg_mgr: &ConfigManager, target_url: &str, mode: MigrationMode) -> CowenResult<()> {
    ...
    async fn migrate_profile(&self, profile: &str, target: &dyn Store) -> CowenResult<()> {
        ...
        // Tokens (Short-lived, we only migrate current access/refresh if present)
