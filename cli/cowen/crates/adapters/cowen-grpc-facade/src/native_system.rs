...
impl NativeSystemService for NativeSystemController {
    type TunnelPluginStream =
        Pin<Box<dyn Stream<Item = Result<TunnelPluginResponse, Status>> + Send>>;

    async fn store_status(
        &self,
        request: Request<StoreStatusRequest>,
    ) -> Result<Response<StoreStatusResponse>, Status> {
        let claims = request
            .extensions()
        ...
        match self
            .capabilities
            .native_system
            .store_status(claims.as_ref(), domain_req)
            .await
        {
    ...
    async fn system_status(
        ...
        match self
            .capabilities
            .native_system
            .system_status(claims.as_ref(), domain_req)
            .await
        {
    ...
    async fn system_reset(
        ...
        match self
            .capabilities
            .native_system
            .system_reset(claims.as_ref(), domain_req)
            .await
        {
    ...
    async fn doctor(
        ...
        match self
            .capabilities
            .native_system
            .doctor(claims.as_ref(), domain_req)
            .await
        {
    ...
    async fn tunnel_plugin(
        ...
        match self
            .capabilities
            .native_system
            .tunnel_plugin(claims.as_ref(), stream)
            .await
        {
            Ok(resp_stream) => {
