...
#[tokio::test]
async fn test_get_token_org_id_only() {
    let tmp = tempfile::tempdir().unwrap();
    let store = Arc::new(cowen_store::FileStore::new(tmp.path().join("vault"), None).unwrap());
    let vault = Arc::new(common::MockVault::with_store(store));
    let pool: Arc<dyn TokenPool> = Arc::new(VaultTokenPool::new(vault));
    ...
    let config = Config::default_with_profile("test");
    let mut headers = reqwest::header::HeaderMap::new();
    headers.insert("x-org-id", "O123".parse().unwrap());

    // Without a valid response from HttpSender, it should fail, but it should try org token.
    let result: CowenResult<Token> = provider.get_token("default", &config, &headers).await;
    assert!(result.is_err());
}

#[tokio::test]
async fn test_get_token_org_and_user_id() {
    let tmp = tempfile::tempdir().unwrap();
    let store = Arc::new(cowen_store::FileStore::new(tmp.path().join("vault"), None).unwrap());
    let vault = Arc::new(common::MockVault::with_store(store));
    let pool: Arc<dyn TokenPool> = Arc::new(VaultTokenPool::new(vault));
    ...
    // Without a valid response from HttpSender, it should fail, but it should try user token.
    let result: CowenResult<Token> = provider.get_token("default", &config, &headers).await;
    assert!(result.is_err());
}

#[tokio::test]
async fn test_get_token_fallback() {
    let tmp = tempfile::tempdir().unwrap();
    let store = Arc::new(cowen_store::FileStore::new(tmp.path().join("vault"), None).unwrap());
    let vault = Arc::new(common::MockVault::with_store(store));
    let pool: Arc<dyn TokenPool> = Arc::new(VaultTokenPool::new(vault));
    ...
    // Without headers, it tries to get app_access_token_fallback.
    let result: CowenResult<Token> = provider.get_token("default", &config, &headers).await;
    assert!(result.is_err());
}
