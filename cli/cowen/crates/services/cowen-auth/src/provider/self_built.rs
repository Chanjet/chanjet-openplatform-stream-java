...
impl SelfBuiltProvider {
    ...
    fn parse_token(val: PlatformTokenValue) -> cowen_common::models::Token {
...
impl AuthProvider for SelfBuiltProvider {
    ...
    async fn generate_auth_url(
        ...
        match self.get_token(profile, config, &reqwest::header::HeaderMap::new()).await {
            Ok(t) => Ok((t.value, "direct".to_string())),
            Err(e) => Err(e),
        }
    }
