...
impl Vault for StoreVault {
    ...
    async fn migrate(&self) -> CowenResult<()> {
        ...
        if self.sensitive.name() != self.primary.name()
            || self.sensitive.description() != self.primary.description()
        {
...
impl TokenDomain for StoreVault {
    ...
    async fn save_app_access_token(
        &self,
        app_key: &str,
        token: cowen_common::models::Token,
    ) -> CowenResult<()> {
...
impl SessionDomain for StoreVault {
    async fn get_session(&self, state: &str) -> CowenResult<cowen_common::models::AuthSession> {
        let json = self
            .sensitive
            .get_token("global", &format!("session:{}", state))
            .await?;
    ...
    async fn save_session(&self, session: cowen_common::models::AuthSession) -> CowenResult<()> {
        ...
        self.sensitive
            .set_token("global", &format!("session:{}", session.state), &json, 3600)
            .await
    }
    async fn delete_session(&self, state: &str) -> CowenResult<()> {
        self.sensitive
            .delete_token("global", &format!("session:{}", state))
            .await
    }
...
impl ConfigDomain for StoreVault {
    ...
    async fn get_config_full(
        ...
        profile: &str,
        key: &str,
    ) -> CowenResult<cowen_common::models::Item> {
    ...
    async fn set_config_conditional(
        ...
        self.primary
            .set_config_conditional(profile, key, value, expected_version)
            .await
    }
