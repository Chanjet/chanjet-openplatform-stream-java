...
impl SqlDriver for PostgresDriver {
    ...
    async fn get_config(&self, profile: &str, key: &str) -> CowenResult<String> {
        let row: (String,) = sqlx::query_as(
            "SELECT item_value FROM cowen_config WHERE profile = $1 AND item_key = $2",
        )
        .bind(profile)
        .bind(key)
        .fetch_one(&self.pool)
    ...
    async fn list_configs(&self, profile: &str) -> CowenResult<Vec<String>> {
        let rows: Vec<(String,)> =
            sqlx::query_as("SELECT item_key FROM cowen_config WHERE profile = $1")
                .bind(profile)
                .fetch_all(&self.pool)
                .await
                .map_err(|e| CowenError::Store(e.to_string()))?;
    ...
    async fn get_secret(&self, profile: &str, key: &str) -> CowenResult<String> {
        let row: (String,) = sqlx::query_as(
            "SELECT item_value FROM cowen_secret WHERE profile = $1 AND item_key = $2",
        )
        .bind(profile)
        .bind(key)
        .fetch_one(&self.pool)
    ...
    async fn set_secret(&self, profile: &str, key: &str, value: &str) -> CowenResult<()> {
        sqlx::query(
            "INSERT INTO cowen_secret (profile, item_key, item_value) VALUES ($1, $2, $3) 
    ...
    async fn list_secrets(&self, profile: &str) -> CowenResult<Vec<String>> {
        let rows: Vec<(String,)> =
            sqlx::query_as("SELECT item_key FROM cowen_secret WHERE profile = $1")
                .bind(profile)
                .fetch_all(&self.pool)
                .await
                .map_err(|e| CowenError::Store(e.to_string()))?;
    ...
    async fn delete_access_token(&self, profile: &str) -> CowenResult<()> {
        sqlx::query(
            "DELETE FROM cowen_tenant_token WHERE profile = $1 AND token_type = 'access_token'",
        )
        .bind(profile)
        .execute(&self.pool)
        .await
    ...
    async fn delete_refresh_token(&self, profile: &str) -> CowenResult<()> {
        sqlx::query(
            "DELETE FROM cowen_tenant_token WHERE profile = $1 AND token_type = 'refresh_token'",
        )
        .bind(profile)
        .execute(&self.pool)
        .await
    ...
    async fn get_app_access_token(&self, app_key: &str) -> CowenResult<Token> {
        let row: (String, DateTime<Utc>, DateTime<Utc>) = sqlx::query_as(
            "SELECT token_value, expires_at, created_at FROM cowen_app_token WHERE app_key = $1",
        )
        .bind(app_key)
        .fetch_one(&self.pool)
        .await
    ...
    async fn get_app_ticket(&self, app_key: &str) -> CowenResult<Ticket> {
        let row: (String, DateTime<Utc>) =
            sqlx::query_as("SELECT ticket_value, created_at FROM cowen_ticket WHERE app_key = $1")
                .bind(app_key)
                .fetch_one(&self.pool)
                .await
                .map_err(|e| match e {
    ...
    async fn get_token(&self, profile: &str, key: &str) -> CowenResult<String> {
        let row: (String,) = sqlx::query_as(
            "SELECT item_value FROM cowen_token WHERE profile = $1 AND item_key = $2",
        )
        .bind(profile)
        .bind(key)
        .fetch_one(&self.pool)
    ...
    async fn list_tokens(&self, profile: &str) -> CowenResult<Vec<String>> {
        let rows: Vec<(String,)> =
            sqlx::query_as("SELECT item_key FROM cowen_token WHERE profile = $1")
                .bind(profile)
                .fetch_all(&self.pool)
                .await
                .map_err(|e| CowenError::Store(e.to_string()))?;
    ...
    async fn clear_profile(&self, profile: &str) -> CowenResult<()> {
        sqlx::query("DELETE FROM cowen_config WHERE profile = $1")
            .bind(profile)
            .execute(&self.pool)
            .await
            .map_err(|e| CowenError::Store(e.to_string()))?;
        sqlx::query("DELETE FROM cowen_secret WHERE profile = $1")
            .bind(profile)
            .execute(&self.pool)
            .await
            .map_err(|e| CowenError::Store(e.to_string()))?;
        sqlx::query("DELETE FROM cowen_token WHERE profile = $1")
            .bind(profile)
            .execute(&self.pool)
            .await
            .map_err(|e| CowenError::Store(e.to_string()))?;
        sqlx::query("DELETE FROM cowen_audit WHERE profile = $1")
            .bind(profile)
            .execute(&self.pool)
            .await
            .map_err(|e| CowenError::Store(e.to_string()))?;
        sqlx::query("DELETE FROM cowen_dlq WHERE profile = $1")
            .bind(profile)
            .execute(&self.pool)
            .await
            .map_err(|e| CowenError::Store(e.to_string()))?;
    ...
    async fn rename_profile(&self, old_name: &str, new_name: &str) -> CowenResult<()> {
        sqlx::query("UPDATE cowen_config SET profile = $1 WHERE profile = $2")
            .bind(new_name)
            .bind(old_name)
            .execute(&self.pool)
            .await
            .map_err(|e| CowenError::Store(e.to_string()))?;
        sqlx::query("UPDATE cowen_secret SET profile = $1 WHERE profile = $2")
            .bind(new_name)
            .bind(old_name)
            .execute(&self.pool)
            .await
            .map_err(|e| CowenError::Store(e.to_string()))?;
        sqlx::query("UPDATE cowen_token SET profile = $1 WHERE profile = $2")
            .bind(new_name)
            .bind(old_name)
            .execute(&self.pool)
            .await
            .map_err(|e| CowenError::Store(e.to_string()))?;
        sqlx::query("UPDATE cowen_tenant_token SET profile = $1 WHERE profile = $2")
            .bind(new_name)
            .bind(old_name)
            .execute(&self.pool)
            .await
            .map_err(|e| CowenError::Store(e.to_string()))?;
        sqlx::query("UPDATE cowen_audit SET profile = $1 WHERE profile = $2")
            .bind(new_name)
            .bind(old_name)
            .execute(&self.pool)
            .await
            .map_err(|e| CowenError::Store(e.to_string()))?;
        sqlx::query("UPDATE cowen_dlq SET profile = $1 WHERE profile = $2")
            .bind(new_name)
            .bind(old_name)
            .execute(&self.pool)
            .await
    ...
    async fn raw_del(&self, key: &str) -> CowenResult<()> {
        sqlx::query("DELETE FROM cowen_config WHERE item_key = $1")
            .bind(key)
            .execute(&self.pool)
            .await
            .map_err(|e| CowenError::Store(e.to_string()))?;
