...
impl cowen_common::store::Store for FileStore {
    ...
    async fn save_user_permanent_code(
        ...
        self.save_raw("perm_user", k, &format!("{}_{}", org, user), c)
    }

    async fn get_token(&self, p: &str, k: &str) -> CowenResult<String> {
        self.load_raw("tokens_legacy", p, k)
    }
    ...
    async fn list_dlq_paged(
        ...
        self.list_all_paged(p, offset, limit)
    }
    ...
    async fn list_all_profiles(&self) -> CowenResult<Vec<String>> {
        Ok(std::fs::read_dir(self.root_dir())
            .map_err(|e| cowen_common::CowenError::Store(e.to_string()))?
            .filter_map(|e| {
                e.ok()
    ...
    fn description(&self) -> String {
        format!("Local File Store at {:?}", self.root_dir())
    }
}
...
impl FileStore {
    ...
    fn load_raw(&self, prefix: &str, profile: &str, id: &str) -> CowenResult<String> {
        ...
        if !path.exists() {
            return Err(cowen_common::CowenError::Store("Not found".into()));
        }
        let content =
            std::fs::read(path).map_err(|e| cowen_common::CowenError::Store(e.to_string()))?;

        let json_bytes = if let Some(fp) = self.fingerprint() {
            let key = cowen_common::security::derive_key(fp);
            cowen_common::security::decrypt(&content, &key)
                .map_err(|e| cowen_common::CowenError::Store(e.to_string()))?
        } else {
            content
        };
    ...
    fn save_raw(&self, prefix: &str, profile: &str, id: &str, data: &str) -> CowenResult<()> {
        ...
        let final_data = if let Some(fp) = self.fingerprint() {
            let key = cowen_common::security::derive_key(fp);
            cowen_common::security::encrypt(data.as_bytes(), &key)
                .map_err(|e| cowen_common::CowenError::Store(e.to_string()))?
        } else {
