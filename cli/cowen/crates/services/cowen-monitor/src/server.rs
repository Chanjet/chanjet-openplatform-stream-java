...
use crate::mgmt::{finalize_auth_handler, progress_handler, AuthManager};
...
impl MonitorServer {
    ...
    fn spawn_telemetry_subscriber(&self) {
    ...
    async fn bind_listener(&self, allow_fallback: bool) -> anyhow::Result<tokio::net::TcpListener> {
            ...
            match bind_result {
                ...
                Err(e) if e.kind() == std::io::ErrorKind::AddrInUse => {
                    if current_port != 0 && retry_count < 2 {
                        ...
                        retry_count += 1;
                    } else if allow_fallback && current_port != 0 {
                        tracing::warn!(target: "sys", "Monitor port {} is in use, falling back to a random port...", current_port);
                        current_port = 0;
                ...
                Err(e) => {
                    return Err(anyhow::anyhow!(
                        "Failed to bind to monitor port {}: {}",
                        current_port,
                        e
                    ))
                }
    ...
    pub async fn start(
        ...
        self.spawn_telemetry_subscriber();
        ...
        let listener = self.bind_listener(allow_fallback).await?;
...
async fn metrics_handler() -> impl IntoResponse {
    let _gauge = crate::gauge!(
        "cowen_active_connections",
