    ...
    match ipc.dlq_list(profile, page, page_size).await {
        Ok(DaemonResponse::DlqData { json }) => {
            ...
            for entry in entries {
                ...
                println!("[ID: {}] [{}] {} - Retry: {}", id, created_at, topic, retry_count);
...
pub async fn view(profile: &str, id: String) -> Result<()> {
    ...
    match ipc.dlq_view(profile, &id).await {
        Ok(DaemonResponse::DlqData { json }) => {
            ...
            println!("  Created:     {}", entry["created_at"].as_str().unwrap_or(""));
            println!("  Retry Count: {}", entry["retry_count"].as_i64().unwrap_or(0));
            println!("--------------------------------------------------");
            println!("\x1b[1;33mPayload:\x1b[0m");
            if let Some(payload_str) = entry["payload"].as_str() {
                match serde_json::from_str::<serde_json::Value>(payload_str) {
                    Ok(parsed_payload) => println!("{}", serde_json::to_string_pretty(&parsed_payload).unwrap_or_default()),
