...
[ -f crates/cowen-cli/tests/e2e/scripts/common.sh ] && source crates/cowen-cli/tests/e2e/scripts/common.sh

# 🚀 All-in-One: Start in-container databases if running inside Podman/Docker
if [ -f /.dockerenv ] || [ -f /run/.containerenv ]; then
    if [ -f crates/cowen-cli/tests/infra/start_services.sh ]; then
        source crates/cowen-cli/tests/infra/start_services.sh
    fi
fi
