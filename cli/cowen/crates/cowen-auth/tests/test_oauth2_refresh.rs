...
impl cowen_common::vault::Vault for MockVault {
    fn primary_store(&self) -> Arc<dyn cowen_store::Store> { unimplemented!() }
}
...
impl TokenDomain for MockVault {
    ...
    async fn get_app_access_token(&self, _: &str) -> CowenResult<Token> { Err(CowenError::Auth("not found".to_string())) }
...
impl ConfigDomain for MockVault {
    async fn get_config(&self, _: &str, k: &str) -> CowenResult<String> {
        self.configs.lock().await.get(k).cloned().ok_or(CowenError::Auth("not found".to_string()))
    }
    ...
    async fn get_config_metadata(&self, _: &str, _: &str) -> CowenResult<(u64, i64)> { Ok((0, 0)) }
    async fn get_config_full(&self, _: &str, _: &str) -> CowenResult<Item> { Err(CowenError::Auth("not found".to_string())) }
...
#[async_trait] impl PermanentCodeDomain for MockVault {
    async fn get_org_permanent_code(&self, _: &str, _: &str) -> CowenResult<String> { Err(CowenError::Auth("not found".to_string())) }
    async fn save_org_permanent_code(&self, _: &str, _: &str, _: &str) -> CowenResult<()> { Ok(()) }
    async fn get_user_permanent_code(&self, _: &str, _: &str, _: &str) -> CowenResult<String> { Err(CowenError::Auth("not found".to_string())) }
    async fn save_user_permanent_code(&self, _: &str, _: &str, _: &str, _: &str) -> CowenResult<()> { Ok(()) }
}
#[async_trait] impl TicketDomain for MockVault {
    async fn get_app_ticket(&self, _: &str) -> CowenResult<Ticket> { Err(CowenError::Auth("not found".to_string())) }
    async fn save_app_ticket(&self, _: &str, _: Ticket) -> CowenResult<()> { Ok(()) }
    async fn delete_app_ticket(&self, _: &str) -> CowenResult<()> { Ok(()) }
}
#[async_trait] impl SessionDomain for MockVault {
    async fn get_session(&self, _: &str) -> CowenResult<AuthSession> { Err(CowenError::Auth("not found".to_string())) }
    async fn save_session(&self, _: AuthSession) -> CowenResult<()> { Ok(()) }
    async fn delete_session(&self, _: &str) -> CowenResult<()> { Ok(()) }
}
#[async_trait] impl SecretDomain for MockVault {
    async fn get_secret(&self, _: &str, _: &str) -> CowenResult<String> { Err(CowenError::Auth("not found".to_string())) }
    async fn set_secret(&self, _: &str, _: &str, _: &str) -> CowenResult<()> { Ok(()) }
    async fn delete_secret(&self, _: &str, _: &str) -> CowenResult<()> { Ok(()) }
}
#[async_trait] impl AuditDomain for MockVault {
    async fn save_audit(&self, _: &cowen_common::models::AuditEntry) -> CowenResult<()> { Ok(()) }
    async fn list_audit(&self, _: &str, _: usize) -> CowenResult<Vec<cowen_common::models::AuditEntry>> { Ok(vec![]) }
}
#[async_trait] impl DlqDomain for MockVault {
...
#[async_trait] impl ManagementDomain for MockVault {
...
impl cowen_auth::client::HttpSender for MockHttpSender {
    async fn post(&self, _: &str, _: reqwest::header::HeaderMap, _: serde_json::Value) -> CowenResult<cowen_auth::client::SimpleResponse> {
        Ok(cowen_auth::client::SimpleResponse { status: 200, body: "{\"access_token\":\"new_at\",\"refresh_token\":\"new_rt\",\"expires_in\":3600}".to_string() })
    }
    async fn post_form(&self, _: &str, _: reqwest::header::HeaderMap, _: serde_json::Value) -> CowenResult<cowen_auth::client::SimpleResponse> {
        Ok(cowen_auth::client::SimpleResponse { status: 200, body: "{\"access_token\":\"new_at\",\"refresh_token\":\"new_rt\",\"expires_in\":3600}".to_string() })
    }
    async fn get(&self, _: &str, _: reqwest::header::HeaderMap) -> CowenResult<cowen_auth::client::SimpleResponse> {
        Ok(cowen_auth::client::SimpleResponse { status: 200, body: "{}".to_string() })
    }
}
...
async fn test_oauth2_refresh_works_with_structured_rt() {
    ...
    vault.save_refresh_token("p1", Token {
    ...
    assert!(res.is_ok(), "Refresh should succeed now with structured RT support: {:?}", res.err());
...
async fn test_oauth2_on_maintenance_tick_refreshes_expired_token() {
    ...
    vault.save_access_token("p1", Token {
    ...
    provider.on_maintenance_tick("p1", &config).await.expect("Maintenance tick failed");
    
    // 3. Check if access token was updated. 
    // EXPECTATION: It SHOULD be updated now.
    let token = vault.get_access_token("p1").await.unwrap();
    assert_eq!(token.value, "new_at", "Token should have been updated to new_at");
}
