...
use crate::client::{HttpSender, ReqwestSender};
...
impl SelfBuiltProvider {
    ...
    async fn perform_network_refresh(&self, profile: &str, cfg: &Config) -> Result<cowen_common::models::Token> {
        ...
        if !resp.is_success() {
            ...
            let safe_err = cowen_common::utils::mask_sensitive_json(&err_text);
...
impl AuthProvider for SelfBuiltProvider {
    ...
    async fn handle_platform_event(&self, _profile: &str, config: &Config, event: crate::provider::PlatformEvent) -> Result<()> {
        match event {
    ...
    async fn get_diagnostics(&self, ctx: &cowen_common::status::StatusContext<'_>) -> Result<Vec<cowen_common::status::StatusEntry>> {
        use cowen_common::status::{StatusEntry, StatusLevel, AsStatusUI, collect_daemon_status};
