...
linux-x86_64-with-podman: prepare-podman-image
	...
	$(PODMAN_RUN_BASE) \
		...
		bash -c "cargo build --release -j $(MAX_PARALLEL) --target x86_64-unknown-linux-gnu -p cowen-cli -p cowen-daemon -p cowen-search-embedding -p cowen-signer && \
			target_podman/x86_64-unknown-linux-gnu/release/cowen-signer sign-plugin --dylib target_podman/x86_64-unknown-linux-gnu/release/libcowen_search_embedding --name cowen-search-embedding --version $(VERSION) --dev-key dist_assets/keys/official_dev.pk8 --dev-cert dist_assets/keys/official_dev_cert.json --out-bundle target_podman/x86_64-unknown-linux-gnu/release/libcowen_search_embedding.bundle --manifest-file crates/plugins/cowen-search-embedding/plugin.json && \
...
windows-plugin-x64:
	...
	powershell -NoProfile -Command "cargo run --release -p cowen-signer -- sign-plugin --dylib target/x86_64-pc-windows-msvc/release/cowen_search_embedding.dll --name cowen-search-embedding --version $(VERSION) --dev-key dist_assets/keys/official_dev.pk8 --dev-cert dist_assets/keys/official_dev_cert.json --out-bundle target/x86_64-pc-windows-msvc/release/cowen_search_embedding.bundle --manifest-file crates/plugins/cowen-search-embedding/plugin.json"
...
windows-plugin-x86:
	...
	powershell -NoProfile -Command "cargo run --release -p cowen-signer -- sign-plugin --dylib target/i686-pc-windows-msvc/release/cowen_search_embedding.dll --name cowen-search-embedding --version $(VERSION) --dev-key dist_assets/keys/official_dev.pk8 --dev-cert dist_assets/keys/official_dev_cert.json --out-bundle target/i686-pc-windows-msvc/release/cowen_search_embedding.bundle --manifest-file crates/plugins/cowen-search-embedding/plugin.json"
