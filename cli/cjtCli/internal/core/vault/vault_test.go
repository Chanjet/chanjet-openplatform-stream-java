...
func TestVaultFallback(t *testing.T) {
	tempDir, err := os.MkdirTemp("", "cjtCli-vault-test-*")
	assert.NoError(t, err)
	...
	v, err := NewVault("cjtCli-test-service", sealPath, "test-master-key")
	assert.NoError(t, err)
	...
	if _, err := os.Stat(sealPath); err == nil {
		...
		v2, err := NewVault("cjtCli-test-service", sealPath, "test-master-key")
		assert.NoError(t, err)
