...
**注意**：macOS 专用的静态库 `libtokenizers.a` 位于 `cli/cjtCli/lib/` 目录下。

```bash
cd cli/cjtCli
CGO_LDFLAGS="-L$(pwd)/lib -L$(pwd)/pkg/search/assets/lib/darwin-arm64" go build -o build/cjtCli-darwin-arm64 ./cmd/cjtCli
```
