...
- [x] **环境变量前缀规范**: 明确定义注入环境变量的命名规范（例如：编译期参数统一使用 `COWEN_BUILD_*`，运行时全局覆盖使用 `COWEN_GLOBAL_*`）。
- [x] **Trait 签名约束**: `Resettable` Trait 必须支持返回将要删除的资源清单 (用于 Dry Run)，例如 `async fn dry_run(&self) -> Vec<String>` 与 `async fn reset(&self) -> Result<()>`。
- [x] **迁移日志标准**: 配置迁移脚本执行时，必须输出标准化的日志，明确告知用户哪些配置项被“上移”至了全局环境，哪些 Profile 配置被废弃。

---

## 7. 附件：v0.3.5 结项报告 (Closing Report)

> [!NOTE]
> 本附件记录了 v0.3.5 版本的正式结项结论与关键技术指标。

### 7.1 核心需求实现与对齐
本版本已实现 PRD 定义的所有核心功能组件：
1. **全局配置层级重构**：网络、日志、安全等配置全部收拢上移至全局 `app.yaml`，并在 Profile 中实现彻底废弃与单向迁移；敏感凭证隔离在 Profile 层。
2. **构建期参数安全注入**：引入 `build.rs` 强制捕获 `COWEN_BUILD_*` 构建变量并用 `env!()` 固化，未提供时立即中断编译。
3. **系统重置 OCP 插件化重构**：定义了 `Resettable` SPI 并结合 `inventory` 静态注册收集器，实现了 `--dry-run` 清单收集和正式清理的二相重置。
4. **IPC / UDS 路径防溢出**：对超长绝对路径加入 SHA-256 唯一哈希缩短机制，确保在各种自定义环境下 IPC 正常通信。

### 7.2 回归验证结论
1. **单元与集成测试**：100% 测试通过。
2. **高并发 E2E 回归测试**：对全量 52 个历史脚本中的 152 处 `exit 1` 退出风格统一升级为规范的 `fail_suite`，经 56 个并行 E2E 用例大回归跑测，整体以 PASSED 状态通过，未发现由于共享存储或端口抢占引起的任何并发冲突，架构极其稳健。

**结论**：各项指标和验收标准已全部达标，正式宣布结项并归档。
